...
const getClass = (request, response) => {
    ...
    switch (type) {
        case "size":
            {
                client.query(
                    `SELECT * FROM classes WHERE size > ${size} ORDER BY name DESC`,
                    (err, res) => {
                        if (!err) {
                            response.status(200).json(res.rows);
                        } else {
        ...
        case "managernot":
            {
                // !=
                client.query(
                    `SELECT * FROM classes WHERE manager <> '${manager}'`,
                    (err, res) => {
                        if (!err) {
                            response.status(200).json(res.rows);
                        } else {
        ...
        case "managerlistnot":
            {
                ...
                console.log(qstring);
                client.query(
                    `SELECT * FROM classes WHERE manager ${qstring} ${list}`,
                    (err, res) => {
                        if (!err) {
                            response.status(200).json(res.rows);
                        } else {
        ...
        case "costFirstIncrease":
            {
                ...
                client.query(
                    `SELECT * FROM classes WHERE cost BETWEEN ${costrange[0]} AND ${costrange[1]} ORDER BY cost ${sortType}`,
                    (err, res) => {
                        if (!err) {
                            response.status(200).json(res.rows);
                        } else {
...
const create = (request, response) => {
    ...
    client.query(
        ...
        [size, cost, manager, usefull, name],
        (err, res) => {
            if (!err) {
